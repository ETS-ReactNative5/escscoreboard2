...
import countries, {all_voters, baseUrl, countryNameMap, edition_id, get_countries, magic_code} from "./constants";
...
export function EurovisionVote() {
  ...
  const sendVote = () => {
    ...
    const finalVotes = votes.map((vote) => {return {"rank":vote.rank, "entry":vote.code.id}})
    ...
    const edition = "esc2022"
      ...
      return (
        <div className={name}>
          ...
          <button className="btn btn--primary" onClick={() => sendVote()}>Submit</button>
        </div>
        );
  }
